...
function List() {
    ...
    const addpro = () =>{
        ...
        const obj = {'id':num , 'name' : document.getElementById('name').value , 'price' : document.getElementById('price').value , 'link' : document.getElementById('link').value
                    ,'desc' : document.getElementById("desc").value , 'category' : document.getElementById("cat").value.toLowerCase()             
    }
        setnum(num+1)
        arr.push(obj)
        setitems(arr)
    ...
    <div className = "List">
        ...
        <Switch>
            <Route exact path = "/item-display">
                <Display item = {it}  items = {items} setitems = {setitems}/>
            </Route>
        </Switch>
        </Router>
